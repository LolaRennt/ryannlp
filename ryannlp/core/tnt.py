            ...
            for s2 in self.state:
                for s3 in self.state:
                    if s3 != u'P':
                        ...
                        if self.uni.getCount(s2) == 0:
                            p2 = 0
                        else:
                            p2 = self.lam2 * self.big.getCount((s2,s3)) / self.uni.getCount(s2)
                        if self.big.getCount((s1,s2)) == 0:
                            p3 = 0
                        else:
                            p3 = self.lam3 * self.tri.getCount((s1,s2,s3)) / self.big.getCount((s1,s2))
        ...
        for word in sentence:
            ...
            for state in stateset:
                ...
                for item in tagpre:
                    ...
                    tagtemp.append([tritutle,probnow])
